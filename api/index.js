...
async function fetchProducts(query) {
    ...
    const url = `http://api.scraperapi.com?api_key=${apiKey}&url=https://www.google.com/search?q=${encodeURIComponent(query)}&tbm=shop`;

    try {
        const response = await fetch(url);
        const html = await response.text();

        // VERY BASIC parsing (we improve later)
        const products = [];

        const regex = /<div class="sh-dgr__content">([\s\S]*?)<\/div>/g;
        let match;
        ...
        while ((match = regex.exec(html)) !== null) {
            ...
            const linkMatch = item.match(/href="\/url\?q=(.*?)&/);
        ...
        return products.slice(0, 10); // limit
    } catch (err) {
        console.error(err);
        return [];
    }
